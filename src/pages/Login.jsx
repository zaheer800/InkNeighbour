...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(false)
    if (err) { setError(t('login.error_invalid')); return }
    navigate('/dashboard')
  }
  ...
  async function handleForgot(e) {
    ...
    setLoading(false)
    if (err) { toast.error('Could not send reset link. Check the email address.'); return }
    setMode('sent')
  }
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-3 pt-8">
            ...
            <h1 className="font-display text-4xl font-black text-white leading-tight">
              Welcome back,<br /><span className="text-orange">Owner.</span>
            </h1>
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:min-h-0">
        ...
        <div className="lg:hidden bg-ink2 px-4 pt-6 pb-10 relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center mb-6">
            <Link to="/" aria-label="Back to home" className="p-2 text-white/60 hover:text-white transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center rounded-xl hover:bg-white/10">
              <ChevronLeft size={22} />
            </Link>
        ...
        <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-8 lg:py-0">
          <div className="w-full max-w-sm">
            ...
            <Link to="/" className="hidden lg:inline-flex items-center gap-1.5 text-muted hover:text-ink text-sm font-medium transition-colors mb-8 min-h-[44px]">
              ...
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    <label htmlFor="login-email" className="text-sm font-semibold text-ink">{t('login.email_label')}</label>
                    <div className="relative">
                      <Mail size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
                      <input id="login-email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required autoComplete="email" autoFocus
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label htmlFor="login-password" className="text-sm font-semibold text-ink">{t('login.password_label')}</label>
                      <button type="button" onClick={() => { setResetEmail(email); setMode('forgot') }} className="text-xs text-violet hover:text-violet/70 font-semibold transition-colors min-h-[36px] px-1">
                        Forgot password?
                      </button>
                    </div>
                    <div className="relative">
                      <Lock size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
                      <input id="login-password" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required autoComplete="current-password"
                        className="w-full min-h-[52px] pl-11 pr-12 py-3 rounded-xl border border-border bg-surface text-base text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-violet/40 focus:border-violet transition-colors" />
                ...
                <div className="mt-6 pt-6 border-t border-border text-center">
                  <p className="text-muted text-[15px]">
                    {t('login.no_account')}{' '}
                    <Link to="/register" className="text-violet font-bold hover:text-violet/80 transition-colors">{t('login.register_link')}</Link>
                  </p>
                </div>
              </>
            )}
            ...
            {mode === 'forgot' && (
              <>
                <button onClick={() => setMode('login')} className="hidden lg:inline-flex items-center gap-1.5 text-muted hover:text-ink text-sm font-medium transition-colors mb-8 min-h-[44px]">
                ...
                <form onSubmit={handleForgot} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-ink">{t('login.email_label')}</label>
                    <div className="relative">
                      <Mail size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
                      <input type="email" value={resetEmail} onChange={e => setResetEmail(e.target.value)} placeholder="you@example.com" required autoFocus
                        className="w-full min-h-[52px] pl-11 pr-4 py-3 rounded-xl border border-border bg-surface text-base text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-violet/40 focus:border-violet transition-colors" />
                    </div>
                  </div>
                  ...
                  <button type="button" onClick={() => setMode('login')} className="w-full text-sm text-muted hover:text-ink transition-colors min-h-[44px]">
                    Back to sign in
                  </button>
                </form>
              </>
            ...
            {mode === 'sent' && (
              <div className="text-center space-y-5">
                ...
                <div>
                  <h2 className="font-display text-2xl font-black text-ink">Check your email</h2>
                  <p className="text-muted text-sm mt-2 leading-relaxed">
                    We sent a password reset link to <strong className="text-ink">{resetEmail}</strong>.
                    Click the link to set a new password.
                  </p>
                </div>
                <button onClick={() => setMode('login')} className="text-sm text-violet font-semibold hover:text-violet/70 transition-colors min-h-[44px]">
                  Back to sign in
                </button>
              </div>
            )}
            ...
            <p className="text-center text-xs text-muted/60 mt-8">
              ...
              <Link to="/terms" className="hover:text-muted transition-colors underline underline-offset-2">Terms</Link>{' '}and{' '}
              <Link to="/privacy" className="hover:text-muted transition-colors underline underline-offset-2">Privacy Policy</Link>
            </p>
          </div>
        </div>
      </div>
